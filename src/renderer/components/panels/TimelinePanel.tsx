...
export function TimelinePanel() {
  ...
  // 카드 클릭 (드래그 없이)
  const handleCardClick = (e: React.MouseEvent, type: 'ad' | 'cc', id: string, tcIn: number) => {
    e.stopPropagation();
      ...
      if (dragging.type === 'ad') {
        if (dragging.mode === 'move') {
          ...
          const newTcIn = Math.max(
            0,
            Math.min(dragging.originalTcOut - fps, dragging.originalTcIn + deltaFrames)
          );
          updateDescription(dragging.id, { tcIn: newTcIn });
        } else if (dragging.mode === 'resize-end') {
          const newTcOut = Math.min(
            duration,
            Math.max(dragging.originalTcIn + fps, dragging.originalTcOut + deltaFrames)
          );
            ...
            >
              {descriptions.map((desc) => (
                <div
                  ...
                  onMouseDown={(e) =>
                    handleDragStart(e, 'ad', desc.id, 'move', desc.tcIn, desc.tcOut)
                  }
                ...
                >
                  ...
                  <div
                    ...
                    onMouseDown={(e) =>
                      handleDragStart(e, 'ad', desc.id, 'resize-start', desc.tcIn, desc.tcOut)
                    }
                  />
                  {/* 오른쪽 리사이즈 핸들 */}
                  <div
                    ...
                    onMouseDown={(e) =>
                      handleDragStart(e, 'ad', desc.id, 'resize-end', desc.tcIn, desc.tcOut)
                    }
                  />
                </div>
              ))}
            ...
            >
              {captions.map((caption) => (
                <div
                  ...
                  onMouseDown={(e) =>
                    handleDragStart(e, 'cc', caption.id, 'move', caption.tcIn, caption.tcOut)
                  }
